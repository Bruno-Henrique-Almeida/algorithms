...
def linear_search(arr: List[int], target: int) -> str:
    '''
    Performs a linear search in a list to find the target value.
...
def main():
    ...
    target_value: int = 60
    result: str = linear_search(arr=input_data, target=target_value)

    logging.info('Linear Search result: %s', result)
